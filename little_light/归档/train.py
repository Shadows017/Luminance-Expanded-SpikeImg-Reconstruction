...
import CompositeReconstructionLoss as crl

# --data_mode 250 --dataset_path /mnt/storage/home/lchen6/lchen6/data/aorta --device cuda:1
def set_parser():
...
# --- Define the loss function, build once (e.g., right after model/optimizer init) ---
loss_fn = crl.CompositeReconstructionLoss(
    dino_ckpt="/mnt/storage/home/lchen6/lchen6/Remote/SwinSFDINOv3/dinov3_b16",  # or HF repo id
    input_size=512,
    w_dino=0.2,     # start small; tune 0.05–0.5 depending on scales
    dino_kwargs=dict(
        lambda_global=1.0, lambda_patch=1.0, lambda_gram=0.5,
        detach_gt=True, gram_max_patches=None,  # e.g., 1024 if Gram is heavy
        token=True  # or use_auth_token=True depending on your transformers
    )
)
...
def train_val(net,epoch,lr,train_loader,val_loader,device,args):
    ...
    for i in range(epoch):
        ...
        for [data_iter,ik_7,ik_14,ik_21,ik_28,ik_35,_] in tqdm(iter(train_loader)):
            ...
            l_smooth = loss(ik_iter,img_pred) + 0.1*(loss(ik_l , im_l) + loss(ik_r , im_r))

            l_dinov3, parts = loss_fn(img_pred, ik_iter, return_parts=True)
            # print("\n")  # formatted
            # print(f"DINOv3 Loss: {l_dinov3.item():.4f}")  # formatted
            # print(f"SmoothL1 Loss: {l_smooth.item():.4f}")  # formatted
            l = l_smooth + l_dinov3.to(l_smooth.device)
        ...
        for [data_iter,ik_iter,_] in tqdm(iter(val_loader)):
            ...
            with torch.no_grad():
                ...
                dinoloss_epoch_test.append(l_dinov3.item())
        ...
        writer.add_scalar('test/dinoloss',l_dinov3.item(),epoch)

        writer.add_scalar('test/psnr',p,epoch)
        writer.add_scalar('test/ssim',s,epoch)
        print("epoch : {}  \ntrain_loss : {}  train_psnr : {}  train_ssim : {} \ntest_loss : {} test_dinoloss : {}  test_psnr : {}  test_ssim : {}".format(
              str(i) , np.mean(loss_epoch_train) , np.mean(psnr_epoch_train) , np.mean(ssim_epoch_train) , np.mean(loss_epoch_test) , np.mean(dinoloss_epoch_test) , np.mean(psnr_epoch_test) , np.mean(ssim_epoch_test)))
